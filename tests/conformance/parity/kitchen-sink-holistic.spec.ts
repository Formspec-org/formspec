...
import { ConformanceRecorder } from '../../e2e/browser/helpers/conformance';
...
const FIXTURE_DIR = path.resolve(__dirname, '../../e2e/fixtures/kitchen-sink-holistic');
...
async function runCheck(
  ...
  try {
    ...
    console.error(`DEBUG: Check FAILED: ${opts.checkId} - ${opts.detail}`);
...
function canonicalizeResponse(response: any): any {
  ...
  if (Array.isArray(clone.validationResults)) {
    clone.validationResults.forEach((r: any) => {
      delete r.kind;
    });
    ...
    try {
      ...
      await runCheck(
        ...
        async () => {
          ...
          await page.fill('textarea[name="notes"]', 'alpha    beta    gamma');
