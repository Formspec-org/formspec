...
- **ADR 0066 — amendment and supersession** `[7 / 6 / 5]` (**35**) — [`thoughts/adr/0066-stack-amendment-and-supersession.md`](thoughts/adr/0066-stack-amendment-and-supersession.md)
  ...
  - **Trellis:** `supersedes_chain_id` reservation + correction/amendment/rescission vectors (one chain); cross-chain supersession runtime + graph export/verifier (sequential follow-on ADR when cross-chain composition lands).
...
- **ADR 0076 — product-tier consolidation** `[8 / 5 / 7]` (**56**) — [`thoughts/adr/0076-product-tier-consolidation.md`](thoughts/adr/0076-product-tier-consolidation.md)
  ...
  - **Landed 2026-04-28 — workspace fully green** (`cargo test --workspace --no-fail-fast --tests` reports **1235 passed / 0 failed**): governance spec absorption (`workflow-governance.md` 727→814; runtime §8.4/§8.5/§9 → §3.8/§11.4/§12.4); AI spec absorption (`ai-integration.md` 680→682; runtime §8.3 + integration §8.4 → §4.6 deontic ordering); sidecar split (`wos-delivery.schema.json` 676 lines merging calendar+notification+correspondence; `wos-ontology-alignment.schema.json` renamed from semantic-profile; `wos-custody-hook-encoding.schema.json` deleted); runtime artifact migration with $defs promotion (`FactsTierRecord`/`MutationSource`/`VerificationLevel`/`CaseFileSnapshot`/`CapabilityInvocationRecord`/`ProvenanceOutcome` into core); tooling consolidation (`wos-tooling.schema.json` 2411 lines absorbing 5 sources via `$views`); **91 fixtures** rewritten to merged shapes (35 root-level + 56 inline_documents-nested); `$wosKernel` → `$wosWorkflow` rename propagated across 105 Rust files; `crates/wos-conformance/src/marker_shim.rs` deleted (no compat shim per owner directive); **6 standalone Rust document types refactored to embedded-block content** per D-1 (`AIIntegrationDocument`/`GovernanceDocument`/`BusinessCalendarDocument`/`NotificationTemplateDocument`/`SignatureProfileDocument`/`IntegrationProfileDocument` — marker fields stripped, type names retained for consumer compatibility); conformance engine extracts embedded blocks via `extract_embedded_block` + `embedded_block_for_role` mapping (kebab-case + camelCase variants); `drift_monitor` / `agent_config` consumers in `wos-runtime/companion.rs` updated to identify by shape (not legacy marker); 3 new lint rules registered + **lint logic implemented + 8 inline tests** + promoted to `Tested` graduation (`WOS-AGENT-XREF-001`, `WOS-SIG-COVER-001`, `WOS-VER-LEVEL-001`); I-001 reanchored to kernel §9.2; `wos.dev` → `wos-spec.org` host unification across all schema $ids; `README.md` + `RELEASE-STREAMS.md` updated with `$wosWorkflow@X.Y` claims-map.
  - **wos-scout review** dispatched and actioned: F1 (standalone-doc legacy pin), F2 (`inline_documents` skipped by migration), F3 (host inconsistency) all resolved. F4 (160 SCHEMA-DOC-001 inner-block leaves) and F6 (pre-existing §-numbering irregularities at `workflow-governance.md` lines 663/734) confirmed honest debt, tracked.
  - **Still open:** step 3a kernel spec absorption (~3000-line editorial pass — Lifecycle Detail + Runtime + Integration content into kernel/spec.md preserving §-numbering); **blocker surfaced**: ADR 0076 D-8 under-specifies destinations for existing kernel §2 (Conformance Classes) and §6 (Impact Level Classification) which are silently displaced when §11→§6 and §12→§2; owner decision needed (candidates: fold both into §1, distribute thematically, or append at end). Step 7-8 Profiles + Companions directory deletion (depends on 3a — once absorption lands, source markdown + schema source files can be `git rm`). Step 9 `COMPATIBILITY-MATRIX.md` rewrite to envelope-version model. Python `tests/schemas/conftest.py` `MARKER_TO_SCHEMA` rewrite (currently still references the old 19-marker map; pytest passes today only because schema files at deleted-paths still exist on disk pending step 7-8 cleanup — the conftest rewrite + schema deletion land together). 160 SCHEMA-DOC-001 violations on `wos-workflow.schema.json` inner-block leaves (gated on PLN-0176..0207 spec absorption — tracked in `EXCLUDED_SCHEMAS`, not hallucinated).
  - **Execution home:** [`wos-spec/TODO.md`](wos-spec/TODO.md) audit log; this ADR's 12-step plan is the source of truth for remaining work.
  - **Gate:** owner decision on D-8 §2/§6 disambiguation unblocks step 3a; everything else flows from there.
